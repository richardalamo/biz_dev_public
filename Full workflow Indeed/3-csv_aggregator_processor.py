...
tools_ = [
    "[\.; ,]AWS[\.; ,]", "[\.; ,]MS Access[\.; ,]", "[\.; ,]Microsoft Access[\.; ,]", "[\.; ,]Azure[\.; ,]", "[\.; ,] C [\.; ,]", "[\.; ,] C,[\.; ,]", "[\.; ,]C++[\.; ,]", "[\.; ,]Cassandra[\.; ,]", "[\.; ,]CircleCI[\.; ,]", "[\.; ,]Cloud[\.; ,]", "[\.; ,]Confluence[\.; ,]", "[\.; ,]Databricks[\.; ,]", "[\.; ,]Docker[\.; ,]", "[\.; ,]EMR[\.; ,]", "[\.; ,]ElasticSearch[\.; ,]",
...
def process_files(files):
    ...
    for filename in files:
        try:
            ...
            df['Job Role'] = ' '.join(filename.split('_')[3:-1]).upper()
            df['Job_ind'] = df['Job Role'].map({'DATA ANALYST': 1, 'DATA ENGINEER': 2, 'DATA SCIENTIST': 3, 'MACHINE LEARNING ENGINEER': 4})
