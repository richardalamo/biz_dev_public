...
def merge_and_save_results(indeed_df, companies_df, matches, previous_scrape_path, dated_csv_file_path):
    ...
    try:
        ...
        with open('log.txt','a') as log_w: # txt append mode
            log_w.write(f'Matches: {indeed_df_matched.shape[0]}\tIndeed total: {indeed_df.shape[0]}\tmatching Rate: {indeed_df_matched.shape[0] / indeed_df.shape[0]}\tDate: {str(date.today())}\n')
