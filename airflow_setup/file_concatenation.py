...
schema = ['name', 'key', 'title', 'location', 'jobType', 'posted', 'days_ago', 'rating', 'experience', 'salary', 'education', 'feed', 'link', 'Tools', 'Soft Skills', 'Industry Skills', 'description', 'search keyword', 'date', 'year', 'month']

# Read each file in the designated folder
for file in glob.glob(f'{input_csv_path}/jobs_detail*.csv'):

    df = pd.read_csv(file)
    # Only if the dataframe is not empty then we add it to our list of dataframes
    if not df.empty:
...
if dfs_list:
    ...
    data = pd.DataFrame({}, columns=schema)
